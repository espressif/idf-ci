...
from gitlab.v4.objects import MergeRequest, ProjectPipeline
...
class ArtifactManager:
    ...
    :ivar gl: GitLab client instance
    :ivar project: GitLab project instance
    """
    ...
    def __init__(self):
        ...
        self.project = self.gl.projects.get(self.settings.gitlab.project)
        if not self.project:
            raise ValueError(f'Project {self.settings.gitlab.project} not found')

    def get_pipeline_by_id(self, pipeline_id: int) -> ProjectPipeline:
    ...
    def download_artifacts(
        ...
        artifact_type: t.Optional[t.Literal['debug', 'flash', 'metrics']] = None,
        ...
        if commit_sha:
            ...
            logger.debug(f'Using commit {commit_sha} specified by commit_sha')
        else:
