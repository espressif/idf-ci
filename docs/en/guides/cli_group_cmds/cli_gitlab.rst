...
When S3 credentials are configured and ``gitlab.artifacts.s3.enable`` is ``true``, the enabled artifact types are stored in S3 as zip files.
...
Download artifacts from S3 storage with ``download-s3-artifacts``:

.. code-block:: bash

    idf-ci gitlab download-s3-artifacts [OPTIONS] [FOLDER]
...
.. code-block:: bash
    ...
    idf-ci gitlab download-s3-artifacts --commit-sha abc123

    # Download only flash artifacts from a specific commit under a specific folder
    idf-ci gitlab download-s3-artifacts --type flash --commit-sha abc123 /path/to/folder

    # Download all artifacts from latest pipeline of current branch
    idf-ci gitlab download-s3-artifacts

    # Download debug artifacts from latest pipeline of a specific branch
    idf-ci gitlab download-s3-artifacts --type debug --branch feature/new-feature
...
.. code-block:: bash
    ...
    idf-ci gitlab download-s3-artifacts --pipeline-id 12345

    # Download only debug artifacts from a specific pipeline under a specific folder
    idf-ci gitlab download-s3-artifacts --pipeline-id 12345 --type debug /path/to/folder

    # Download flash artifacts from a specific pipeline
    idf-ci gitlab download-s3-artifacts --pipeline-id 12345 --type flash

Artifact Type Details
---------------------

Artifact types are defined under ``gitlab.artifacts.s3.configs``. By default, the following types are configured with ``base_dir_pattern = "**/build*/"``. Each matching base directory generates a ``<artifact_type>.zip`` containing the matching files.
...
Upload artifacts to S3 storage with ``upload-s3-artifacts``:

.. code-block:: bash

    idf-ci gitlab upload-s3-artifacts [OPTIONS] [FOLDER]
...
.. code-block:: bash
    ...
    idf-ci gitlab upload-s3-artifacts --type debug --commit-sha abc123

    # Upload flash artifacts from a specific directory
    idf-ci gitlab upload-s3-artifacts --type flash --commit-sha abc123 /path/to/build
...
Internally, the artifact commands use the ``S3ArtifactManager`` class, which handles:
