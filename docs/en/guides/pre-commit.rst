...
For example, if a test case specifies ``release`` config, the hook checks that a file like ``sdkconfig.ci.release`` or ``sdkconfig.defaults.release`` exists.
...
.. code-block:: text
    ...
    examples/my_project
        Sdkconfig file "release" is missing for test case "test_feature_a"
        Some of sdkconfig files ("debug", "perf") are missing for test cases:
            - test_feature_b
            - test_feature_c
